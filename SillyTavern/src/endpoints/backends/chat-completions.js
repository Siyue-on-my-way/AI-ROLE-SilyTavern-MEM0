...
function joinUrl(...parts) {
    ...
    const first = normalized[0].replace(/\/+$/, '');
    const rest = normalized.slice(1).map(p => p.replace(/^\/+/, '').replace(/\/+$/, ''));
...
function getMem0Settings(request) {
    const base = {
        enabled: getConfigValue('mem0.enabled', false, 'boolean'),
        baseUrl: getConfigValue('mem0.baseUrl', 'http://localhost:8001'),
        timeoutMs: getConfigValue('mem0.timeoutMs', 1200, 'number'),
        limit: getConfigValue('mem0.limit', 6, 'number'),
        maxChars: getConfigValue('mem0.maxChars', 2000, 'number'),
        threshold: getConfigValue('mem0.threshold', undefined, 'number'),
        rewriteEnabled: getConfigValue('mem0.rewrite.enabled', false, 'boolean'),
        rewritePrompt: String(getConfigValue('mem0.rewrite.prompt', 'Rewrite the following conversation context and the last user question into a single, self-contained search query for a memory database. The query should capture the user\'s intent and any relevant details from the context. Do not include any explanation or additional text, just the rewritten query.\n\nContext:\n{{context}}\n\nUser Question: {{query}}')),
    };

    const override = request?.body?.mem0 && typeof request.body.mem0 === 'object' ? request.body.mem0 : {};
    ...
    return {
        ...
        rewriteEnabled: override.rewriteEnabled ?? base.rewriteEnabled,
        rewritePrompt: override.rewritePrompt ?? base.rewritePrompt,
    };
}

function getMem0Identity(request) {
    const userId = String(request?.user?.profile?.handle || '');
    const groupChatId = String(request?.body?.group_chat_id || '');
    if (groupChatId) {
        return { userId, agentId: `groupChat:${groupChatId}` };
    }
    const groupNames = Array.isArray(request?.body?.group_names) ? request.body.group_names.filter(Boolean) : [];
    const charName = String(request?.body?.char_name || '');
    const agentId = groupNames.length ? `group:${groupNames.join('|')}` : (charName ? `char:${charName}` : '');
...
function insertSystemMessageAfterLeadingSystem(messages, systemMessage) {
    ...
    next.splice(insertAt, 0, systemMessage);
    return next;
}

function formatMem0Memories(memories, maxChars) {
    const memoryStrings = memories
        .map(m => {
            if (!m || typeof m !== 'object') return null;
            if (typeof m.memory === 'string' && m.memory.trim()) return m.memory.trim();
            if (m.data?.memory && typeof m.data.memory === 'string' && m.data.memory.trim()) return m.data.memory.trim();
            return null;
        })
        .filter(Boolean);
    ...
    const deduped = [];
    const seen = new Set();
    for (const item of memoryStrings) {
...
async function searchMem0(request, query) {
    const settings = getMem0Settings(request);
    if (!settings.enabled) return null;
    if (!query) return null;

    const { userId, agentId } = getMem0Identity(request);
    ...
    try {
        const body = {
            ...
            limit: Number(settings.limit) || undefined,
            threshold: typeof settings.threshold === 'number' ? settings.threshold : undefined,
        };

        const res = await fetch(joinUrl(String(settings.baseUrl || '').replace(/\/+$/, ''), '/search'), {
            method: 'POST',
...
router.post('/status', async function (request, statusResponse) {
    ...
    try {
        const modelsUrl = new URL(joinUrl(apiUrl, '/models'));
...
router.post('/generate', async function (request, response) {
    ...
    const maybeMem0Inject = async () => {
        if (!Array.isArray(request.body.messages)) return;
        if (String(request.body.type || '') === 'quiet') return;

        let query = getLastUserText(request.body.messages);
        if (!query) return;

        let results = await searchMem0(request, query);

        // Fallback 1: Context Enhancement
        if ((!Array.isArray(results) || results.length === 0)) {
            const contextMessages = request.body.messages
                .slice(-6, -1) // Last 5 messages excluding current user message
            ...
            if (context) {
                ...
                if (enhancedQuery.length <= 1200) { // Limit context length
                    results = await searchMem0(request, enhancedQuery);
                }
        ...
        const settings = getMem0Settings(request);
        const memoriesText = formatMem0Memories(results, settings.maxChars);
