...
router.post('/get', (request, response) => {
    ...
    // Load user themes
    const userThemes = readAndParseFromDirectory(request.user.directories.themes);
    // Load default themes
    const defaultThemesPath = path.join(serverDirectory, 'default', 'content', 'themes');
    const defaultThemes = fs.existsSync(defaultThemesPath) ? readAndParseFromDirectory(defaultThemesPath) : [];
    // Merge: prioritize user themes, add default themes not already present
    const userThemeNames = new Set(userThemes.map(t => t.name));
    const mergedThemes = [...userThemes];
    defaultThemes.forEach(dt => {
        if (!userThemeNames.has(dt.name)) {
            mergedThemes.push(dt);
        }
    });
