...
function getMem0IdentityForChatSave(request, chatData) {
    ...
    if (isGroup) {
        const groupChatId = String(request?.body?.id || '');
        return { userId, agentId: groupChatId ? `groupChat:${groupChatId}` : '' };
    }

    const charName = String(request?.body?.character_name || chatData?.[0]?.character_name || '').trim();
    const avatarDerived = String(request?.body?.avatar_url || '').replace('.png', '').trim();
    const agentId = charName ? `char:${charName}` : (avatarDerived ? `char:${avatarDerived}` : '');
...
function getMem0WriteMarker(chatData) {
    ...
    return String(last?.send_date || '');
}

async function maybeWriteMem0FromChatSave(request, chatData) {
    ...
    try {
        const metadata = {
            source: 'sillytavern',
            chat_type: chatData?.[0]?.is_group ? 'group' : 'character',
            character_name: String(request?.body?.character_name || header?.character_name || ''),
            avatar_url: String(request?.body?.avatar_url || ''),
            group_chat_id: chatData?.[0]?.is_group ? String(request?.body?.id || '') : undefined,
        ...
        const res = await fetch(joinUrl(String(settings.baseUrl || '').replace(/\/+$/, ''), '/memories'), {
            method: 'POST',
