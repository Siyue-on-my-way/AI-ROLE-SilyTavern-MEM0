...
### 5.1 需求背景
为了让用户能灵活控制“对话”与“记忆”分别使用的大模型（例如对话用 GPT-4o，记忆提取用低成本的 gpt-4o-mini 或本地模型），需要在前端提供统一的配置入口。

### 5.2 交互流程

1. **初始化/加载**
   - 前端加载时，调用 Mem0 `GET /models` 获取推荐/支持的模型列表。
   - 读取 SillyTavern 本地配置，回显当前选中的 Chat Model 和 Memory Model。

2. **用户切换模型**
   - **Chat Model 变更**：直接更新 SillyTavern 的 `config.main_api` 或 `config.openai.model` 等相关设置。
   - **Memory Model 变更**：
     - 前端发送 `POST /configure` 到 Mem0 服务。
     - Payload: `{ "llm": { "provider": "openai", "config": { "model": "new-model-name" } } }`
     - Mem0 后端更新内存中的 `MEMORY_INSTANCE`，后续的写入/检索操作即刻生效。

### 5.3 数据流向
- **Frontend (ST)** -> **Backend (Mem0)**: 配置更新指令 (HTTP POST)。
- **Backend (Mem0)**: 内部状态热更新，无需重启容器。
